...
The `gcn1` GPU machine is available for quick GPU tests without the need to request a GPU. However, this node has limited resources. For reference examples, check out [Using IDEs](https://servicedesk.surf.nl/wiki/display/WIKI/PyCharm+and+other+JetBrains+IDEs+for+remote+development) and [Remote Visualization](https://servicedesk.surf.nl/wiki/display/WIKI/Remote+visualization+desktop+on+Snellius).

To connect to the `gcn1` GPU machine, run:

```bash
ssh gcn1
```

If get the error, `Certificate invalid`, remove the old known-hosts entry manually from `.ssh/known_hosts` or by command:
